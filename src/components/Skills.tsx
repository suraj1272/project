...
import { Code, Brush, BarChart3, Globe } from 'lucide-react';
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          <p className="text-xl text-center text-slate-700 dark:text-slate-300 max-w-3xl mx-auto mb-16">
            Here are some of the skills I've developed throughout my academic and personal journey.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {skillCategories.map((category, index) => (
            <div 
              key={category.title}
              className={`bg-white dark:bg-slate-800 rounded-xl shadow-xl p-6 transition-all duration-1000 delay-${index * 200} transform ${
                isInView ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'
                  ...
                  <div key={skill.name} className="space-y-2">
                    ...
                    <div className="w-full h-2 bg-slate-200 dark:bg-slate-700 rounded-full overflow-hidden">
                      <div 
                        ...
                        style={{ 
                          width: `${skill.level}%`,
                          animation: `growWidth 1.5s ease-out ${(skillIndex * 0.2) + (index * 0.5)}s both`
                        }}
                      ></div>
                    </div>
                  </div>
